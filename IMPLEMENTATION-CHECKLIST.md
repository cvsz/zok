...
- [x] PostgreSQL persistence boundary for legacy chat metadata/unread/tags. Deterministic mapping persists `legacyChatId`, avatar, assignment, tags, orders, unread, and display-only time in tenant-scoped contact metadata; request-bound runtime supports metadata reads, mark-read, and tag replacement. Evidence: test-first `7a7b8c8c56c960b405ab63738b9f1a0648ac5021`; mapping `191bdd028502382f52894c8a8cb5c592686c1bf4`; repository `b474ad078147d510a49b5bc65c314cd6c7aba259`; runtime `357c58128dce60370e61be1e1a40acaf479f61c5`; compatibility tests `285696cdc33f8f43a137fa76620fcbca7c863285`, `bd7fefb4d410afdd01eb0cd41a05cfdd0a28808f`; service-backed tenant-isolation regression `19852412c602756af826a10c8541265cea10620d`; implementation-head CI `32393891922`.
- [ ] Wire metadata/unread/tags boundary into configuration-gated Express routes with service-backed API compatibility and rollback regression.
...
PR #17 remains draft/open/unmerged on `feat/postgres-chat-import`, based on main `29f0055d439fda5cf5ac8bab5d8755b371be1817`. The metadata boundary is verified against PostgreSQL with a non-superuser/NOBYPASSRLS role, including cross-tenant invisibility, but live Express metadata/read/tag side effects still use JSON. `ZOK_CHAT_STORAGE=json` remains default and rollback.

## Next bounded unit

- [ ] Wire only legacy chat metadata/unread/tags through the verified PostgreSQL runtime when `ZOK_CHAT_STORAGE=postgres`; preserve auth/CSRF/validation/API shape, JSON default/rollback, and add service-backed API compatibility + rollback tests. Do not migrate unrelated resources or claim production canary/cutover evidence.
