...
- PostgreSQL-mode chat GET reads now consume the verified metadata boundary through the existing route overlay. Metadata mutation routes and message-side unread/display-time effects remain JSON-backed; JSON remains the default/rollback runtime.
...
- PostgreSQL-mode chat GET metadata is verified, but `/api/chats/:id/read`, `/api/chats/:id/tags`, and message-side unread/display-time mutations remain JSON-backed.
